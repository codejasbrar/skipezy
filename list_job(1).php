...
<body style="font-family:Montserrat; font-size:16px;">
    ...
    <input type="hidden" name="filter_search" value="yes">
      ...
      <div class="col-md-12" style="padding-top: 10px;margin-top:20px;box-shadow: 
5px 5px 5px; background-color:#577492;color:white;">


        <div class="col-md-2">
         <label for="job_type">From</label>
         <div class="form-group">
                      
 <input type="text" name="from" class="from form-control" placeholder="Select a Date" id="from">
             
         </div>
        </div>
       <div class="col-md-2">
         <label for="job_type">To</label>
         <div class="form-group">
                      
 <input type="text"  name="to" class="to form-control" placeholder="Select a Date" id="to">
             
         </div>
        </div>
     <div class="col-md-1">
      <label for="paid">Job id</label>
            <div class="form-group">
                  <input type="text" name="job_id" id="job_id" class="job_id form-control">
            </div>      
    
         </div>
         
         <div class="col-md-1">
      <label for="paid">Job Type</label>
            <div class="form-group">
...
id="paid" class="paid job_type form-control">
         </div>
      </div>
      <div class="col-md-2">
          <label style="margin-left:">Post Code</label>
          <div class="form-group">
...
id="name" class="name form-control">
          </div> 
      </div>
...
echo '<p style="text-align:center;background-color:#00DFD4; color:#000000; padding:15px; font-size:18px;">Total '.$rowcount.' jobs are starting today ('.$today.'). You can see other jobs by applying filter. </p>';
?>
                ...
                while($job=mysqli_fetch_assoc($res))

                  {
					  ...
					  if($job['status']=='Job Done'){?>
                        ...
                        <tr class="job_type <?php echo $job['job_type'] ;?>" style="cursor:pointer; background-color:#63EC62;">
                        <?php 
					  }else{?>
							<tr class="job_type <?php echo $job['job_type'] ;?>" style="cursor:pointer; background-color:#ED3B3E;">
							<?php }?>
                           <td><?php
						    echo $job['id']; ?></td>
...

{
	echo "This job is not done";
			  ...
			  ?>
               ...
               <td style="cursor:pointer; background-color:#ED3B3E;"><?php echo $job['payment_type'];?></td>  
...
$('body').delegate('.post_code,.name,.job_type,.paid,.from,.to,.job_id','keyup',function()  
{
  ...
  if (search.length > 0) {
   ...
   var dataString ='filter_search=' + filter_search+'&from=' + from+'&post_code=' + post_code+'&name=' + name+'&job_type=' + job_type+'&paid=' + paid +'&to=' + to+ '&job_id=' + job_id;
